...
sed -i bkp "s/{{APP_URL}}/$APP_URL_SED/g" main.js
check_error $? "erro ao definir url no index.html"
sed -i bkp "s/{{APP_URL}}/$APP_URL_SED/g" src/view.js
check_error $? "erro ao definir url no src/view.js"
sed -i bkp "s/{{ICON}}/$ICON_SED/g" src/window.js
check_error $? "erro ao definir icon no src/window.js"

sed -i bkp "s/{{APP_NAME}}/$APP_NAME/g" package.json
check_error $? "erro ao definir url no package.json"

sed -i bkp "s/{{APP_NAME}}/$APP_NAME/g" app.desktop
check_error $? "erro ao definir nome do app no app.desktop"
sed -i bkp "s/{{APP_DIR}}/$APP_DIR_SED/g" app.desktop
check_error $? "erro ao definir nome do app no app.desktop"
sed -i bkp "s/{{ICON}}/$ICON_SED/g" app.desktop
check_error $? "erro ao definir icon do app no app.desktop"


electron-packager . --overwrite --platform=darwin --arch=x64 --icon=assets/icons/icon.png --prune=true --out=$APP_DIR/
